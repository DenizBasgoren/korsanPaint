...
struct Text : Shape  {
	int i;
	std::string t;
	Text(char c, int x1, int y1);
	void add(char c);
	void draw();
	bool in(int cx, int cy);
};

Text::Text(char c, int x1, int y1) {
	x = x1;
	y = y1;
	t.push_back(c);
	i = 1;
...
int main() {
	...
	font = TTF_OpenFont("/usr/share/fonts/noto/NotoSans-Bold.ttf", 40);
